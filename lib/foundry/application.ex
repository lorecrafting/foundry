defmodule Foundry.Application do
  ...
  def start(_type, _args) do
    ...
    operator_root = Foundry.RuntimeRoot.initialize_operator_root!()
    Foundry.RuntimeRoot.resolve_and_publish!(mix_env, operator_root)
    ...
    if mode == :client and
       ...
       do:
         IO.puts(
           "Foundry: FOUNDRY_MANUAL_LANE is not 1; starting no children (use bin/foundry-lane)"
         )
  ...
  def runtime_children(:lane) do
    IO.puts("Foundry starting: manual lane")
    [Foundry.ManualLane.Server]
  end
  ...
  def manual_lane_enabled? do
    System.get_env("FOUNDRY_MANUAL_LANE") == "1" ||
      Keyword.get(Application.get_env(:foundry, :manual_lane, []), :enabled, false)
  end
