defmodule Foundry.CI do
  ...
  def plan(root, run_root) do
    ...
    [
      ...
      %{
        ...
        artifact: Path.join(run_root, "artifacts/foundry")
      }
    ]
  end
  ...
  defp command_receipt(%{argv: [executable | args]} = command, root, env, run_root) do
    ...
    after
      if command[:artifact], do: File.rm(Path.join(root, "foundry"))
    end
  end
  ...
  defp dependency_skeleton(root) do
    ...
    escript = git_lines(root, ["ls-files", "foundry"])
  ...
  defp resolved_dependencies(root, env, run_root) do
    with_process_environment(env, fn ->
      File.cd!(root, fn ->
        ...
        Mix.Project.in_project(:foundry, root, [env: :test], fn _module ->
    ...
    case status do
      {:ok, dirty} ->
        %{
          ...
          runner_sha256: safe_value(fn -> file_sha(root, "lib/foundry/ci.ex") end),
          workflow_sha256:
  ...
  defp artifact_provenance(run_root, output_dir, root) do
    source = Path.join(run_root, "artifacts/foundry")

    if File.regular?(source) do
      destination = Path.join(output_dir, "foundry")
