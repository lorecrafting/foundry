...
- lib/foundry/preparation.ex
- lib/foundry/relocation/local_exclude.ex
- test/foundry/policy_test.exs
- test/foundry/relocation/local_exclude_test.exs
Three in-scope paths untouched (relocation.ex, preparation_test.exs, relocation_test.exs). No out-of-scope edits.

## Acceptance

1. Preparation deps.get cwd "." — lib/foundry/preparation.ex:6. Test test/foundry/policy_test.exs:19 `Enum.all?(commands, &(&1.cwd == "."))` is exact; red control below shows it fails on "workflow". Met.
2. LocalExclude default "local/" — lib/foundry/relocation/local_exclude.ex:12; moduledoc/doc lines 5, 8, 78, 171 updated. Tests at local_exclude_test.exs:76 (`Enum.count(lines, &(&1 == "local/")) == 1`) and :103/:112 (`"local/" in String.split(content, "\n")`) are exact-line checks; red control shows 2 of 3 tests fail on the old pattern. Met.
3. Database checks (ecto.create at ".") unchanged — preparation.ex:8-11 identical to base. Independent finding: `Preparation.commands/1` has no callers in lib/ at all (only test/foundry/preparation_test.exs and policy_test.exs), so no Foundry ticket reaches either the workflow or the database checks today. The developer's report on this criterion should say the same; if it claims a runtime path exists, that claim is wrong.
...
F1 (low, non-blocking, follow-up) — lib/foundry/relocation/local_exclude.ex:12 vs /.gitignore:6
...
F3 (info) — test/foundry/relocation/local_exclude_test.exs:47,62,85 use `String.contains?(status, "local/")` (substring). Fixture has no other path containing `local/`, and the exact-line assertions at :76/:103/:112 carry the default-pattern check, so this is not a gap; red control confirms :62 fails on the old default.

## Checks run

- Focused green: `TMPDIR=/private/tmp MIX_ENV=test MIX_DEPS_PATH=/Users/raymondluong/dev/foundry/deps mix test test/foundry/preparation_test.exs test/foundry/policy_test.exs test/foundry/relocation/local_exclude_test.exs test/foundry/relocation_test.exs` → 7 passed, 1 skipped, 0 failures.
