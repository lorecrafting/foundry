...
All commands run from the root of the main checkout. Every `bin/foundry lane …` command
...
- The lane daemon is the only daemon: it runs `ManualLane.Server` alone and launches no
  ...
  ([plan amendment C1](../REPAIR-PLAN.md#clean-room-amendment)); `bin/foundry` commands other
...
| `RELEASE_NODE` | `foundry_lane` | node name `bin/foundry lane` targets |
| `FOUNDRY_RUNTIME_ROOT` | `~/.local/state/foundry-lane` | runtime root; the store is `state/manual-lane/authority.sqlite3` |
...
```sh
bin/foundry lane admit ML-42 --base-ref "$(git rev-parse HEAD)" --title "Short title" \
...
bin/foundry lane integrated ML-42 [--ref main]
```
...
```sh
bin/foundry lane status [ML-42]    # phase, attempts, executions; awaits_operator marks issued claims
bin/foundry lane log [ML-42]       # committed events, refused commands, each effect's claim
bin/foundry lane integrated ML-42 [--ref main]   # is base..candidate in the ref?
```
...
```sh
bin/foundry lane recover --evidence "killed pid N at HH:MM; one foundry_lane node running"
