...
| `check_local_layout.exs` | A read-only inventory of legacy-layout paths | `elixir bin/check_local_layout.exs [--root DIR]` | `test/local_layout_test.exs` | **pass** on the worktree, but vacuously: a worktree has no legacy data. With `--root` set to the main checkout it **exits 2 with 5 REVIEW paths**: `raw`, `priv/models`, `priv/embed/.venv`, `priv/plts` and `sources/local/huang-nianzu-jie/text`. The script is working; those paths are an operator finding for [LAYOUT_MIGRATION.md](https://github.com/lorecrafting/pramana/blob/e1e4b3bf2c666f5d84652758afee446a0b21ebe1/docs/LAYOUT_MIGRATION.md) |
