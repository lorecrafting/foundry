defmodule Foundry.ManualLane.ServerTest do
  @moduledoc """
  ...
  describe "flag on" do
    ...
    test "a seed interrupted after the policy is completed by the next start", ctx do
      ...
      assert {:ok, %{"disposition" => "accepted"}, _} =
               Foundry.ManualLane.Replay.root(lane, "manual-lane-seed", "interrupted", %{
                 "type" => "set_policy",
  ...
  defp write_seed!(root, seed) do
    ...
    {:ok, encoded} = Foundry.DurableStore.Encoding.json(seed)
