...
defmodule Foundry.ArchitectureBoundaryTest do
  @moduledoc """
  ...
  @core "lib/foundry/durable_store/**/*.ex"
  @workflow "lib/foundry/workflow/**/*.ex"
  @generic_kernel [
  ...
  describe "rule 1" do
    ...
    test "negative control: a comment or @moduledoc mention is not a reference" do
      path =
        fixture(
          "defmodule C do\n @moduledoc \"Foundry.Workflow.Kernel\"\n # Foundry.Workflow.Kernel\nend"
        )
  ...
  describe "rule 2" do
    ...
    test "red control: a multi-alias of DurableStore is seen" do
      path = fixture("alias Foundry.DurableStore.{Gateway, Database}\nGateway.put(x)")
  ...
  describe "rule 4" do
    ...
    test "red control: a Software reference outside the declared sites is seen" do
      path =
        fixture(
          "defmodule Foundry.Workflow.Kernel do\n" <>
            " alias Foundry.Workflow.Kernel.Software.Review\n" <>
            " @families [Review]\n def f, do: Review.x()\nend"
        )
      ...
      assert [{^path, 4, {Foundry.Workflow.Kernel, {:def, :f}}, _}] = found
    end
  end
  ...
  describe "rule 3" do
    ...
    test "red control: a role literal in a new Core function is seen" do
      ...
      assert [
               {^path, 4, {Foundry.DurableStore.ProtectedPrimitives, {:def, :new_site}},
                "reviewer"}
             ] = role_tokens([path])
    end
